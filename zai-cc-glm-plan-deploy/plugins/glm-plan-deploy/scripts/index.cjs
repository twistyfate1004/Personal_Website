    ...
    (function(global2, factory) {
      ...
      function promiseCallback() {
        ...
        function callback(err, ...args2) {
          if (err) return reject2(err);
          resolve2(args2.length > 1 ? args2 : args2[0]);
        }
      ...
      function queue$1(worker, concurrency, payload) {
        ...
        function _insert(data, insertAtFront, rejectOnError, callback) {
          ...
          function promiseCallback2(err, ...args2) {
            ...
            if (args2.length <= 1) return res(args2[0]);
            res(args2);
          }
...
var programDir = (0, import_path.dirname)(process.argv[1]);
...
var overrideConsole = (ignoreLevels) => {
  if (!originalConsole) {
...
var settingsFile = ".zai/deploy/settings.json";
var updateDeploySettings = (newSettings) => {
  try {
    (0, import_fs.mkdirSync)((0, import_path2.join)(process.cwd(), (0, import_path2.dirname)(settingsFile)), { recursive: true });
    const settingsString = JSON.stringify(newSettings);
    (0, import_fs.writeFileSync)((0, import_path2.join)(process.cwd(), settingsFile), settingsString, "utf-8");
  } catch (error) {
    console.error(`Error writing ${settingsFile}:`, error);
  }
};
var isInvalidProjectIdError = (error) => {
  if (error instanceof AggregateError) {
    if (error.errors[0]?.code === 1220 && (error.errors[0]?.msg === "Invalid projectId" || error.errors[0]?.msg === "Project disabled")) {
      return true;
    }
  }
  return false;
};
var resetDeploySettings = (deploySettings) => {
...
var loadDeploySettings = () => {
  try {
    const projectName = (0, import_path2.basename)(process.cwd());
    const settingsPath = (0, import_path2.join)(process.cwd(), settingsFile);
    let deploySettings;
    try {
...
async function zipFiles(targetFile, files) {
  return new Promise((resolve2, reject) => {
    ...
    output.on("close", () => {
      resolve2(new File([(0, import_fs.readFileSync)(targetFile)], (0, import_path2.basename)(targetFile), { type: "application/zip" }));
    });
    ...
    for (const filePath of files) {
      if ((0, import_fs.existsSync)(filePath)) {
        const relativePath = (0, import_path2.relative)(process.cwd(), filePath);
...
var api_base_url = removeAnthropicPath(process.env.API_BASE_URL || process.env.ANTHROPIC_BASE_URL || "");
var putObjectForDeploy = async (url, file) => {
...
var getUploadParamsForDeploy = async (projectId) => {
...
var getAvailableNodejsVersions = async () => {
...
var getDeployStatus = async (projectId, deployId) => {
...
var getDeployLog = async (projectId, deployId) => {
...
async function deploy(ignoreFilePath, extraParam) {
  try {
    ...
    const files = await filterDirectoryFiles(localFolder, ignoreFilePath, !!outdir ? [outdir] : []);
    console.log(`Filtered files count: ${files.length}`);
    const tmpDir = (0, import_path3.join)(localFolder, ".tmp");
    if (!(0, import_fs2.existsSync)(tmpDir)) {
      (0, import_fs2.mkdirSync)(tmpDir, { recursive: true });
    }
    const tmpZipFn = (0, import_path3.join)(tmpDir, `${Date.now()}_upload.zip`);
    ...
    try {
      ...
      const uploadParams = await getUploadParamsForDeploy(deploySettings.projectId);
      area = uploadParams.area;
      if (area) {
        console.info("Project Area:", area);
      }
      await putObjectForDeploy(uploadParams.presignedUploadUrl, file);
      ...
      const deploymentResult = await deployProject(deployParams);
      ...
      if (isInvalidProjectIdError(error)) {
        resetDeploySettings(deploySettings);
    ...
    let status = null;
    while (true) {
      status = await getDeployStatus(deploySettings.projectId, deployId);
...
async function getLog() {
  try {
    ...
    return await getDeployLog(deploySettings.projectId, deploySettings.deployments[0].deployId);
  } catch (error) {
    return handleUncaughtError(error);
  }
}
...
async function getNodeVersions() {
  try {
    return await getAvailableNodejsVersions();
  } catch (error) {
    return handleUncaughtError(error);
  }
}
...
async function getStatus() {
  try {
    ...
    try {
      const status = await getDeployStatus(deploySettings.projectId, deploySettings.deployments[0].deployId);
      return {
        success: true,
        status: status.status,
        url: status.status === "Success" ? status.result?.previewUrl : void 0,
        message: status.result?.message || "",
        log: status.result?.deployLog,
        audit: status.result?.auditDetail
      };
...
async function zip(targetPath, ignoreFilePath, outDir) {
  try {
    ...
    const zip2 = await zipFiles(
      targetPath,
      await filterDirectoryFiles(process.cwd(), ignoreFilePath, !!outDir ? [outDir] : [])
    );
    return {
      success: true,
      status: "zipped size:" + zip2.size,
...
async function main() {
  ...
  overrideConsole(["LOG", "DEBUG"]);
  try {
    switch (command) {
      case "deploy": {
        const extraParams = JSON.parse(args[1] || "{}");
        const result = await deploy(`${programDir}/.ignore`, extraParams);
        if (result.success) {
          console.info("\u2705 Deploy successful!");
          console.info("\u{1F4CD} URL:", result.url);
        } else {
          console.error("\u274C Deploy failed:", result.message);
          if (result.log) {
            console.error("\u{1F4CD} Deploy log:", result.log);
          }
          if (result.audit) {
            console.error("\u{1F4CD} Audit result:", result.audit);
          }
          process.exit(1);
        }
        break;
      ...
      case "status": {
        ...
        if (result.success) {
          ...
          console.info("\u{1F4CD} Status:", result.status);
          if (result.url) {
            console.info("\u{1F4CD} URL:", result.url);
          }
          console.info("\u{1F4CD} Message:", result.message);
        } else {
      ...
      case "log": {
        ...
        if (result) {
          console.info("\u{1F4CD} Deploy log:", result);
        } else {
      ...
      case "node": {
        const result = await getNodeVersions();
        console.info(result);
        break;
      }
      case "zip": {
        ...
        const outDir = args?.[2];
        const result = await zip(targetPath, `${programDir}/.ignore`, outDir);
        if (result.success) {
